...
export const data = new SlashCommandBuilder()
    .setName("info")
    .setDescription("Answers to all your burning questions...")
    .addSubcommand((subcommand) =>
        subcommand.setName("addons").setDescription("How do I know what addons are required for raiding?"),
    )
    .addSubcommand((subcommand) =>
        subcommand.setName("attendance").setDescription("How do I post (or delete) an absence?"),
    )
    .addSubcommand((subcommand) => subcommand.setName("corkboard").setDescription("How do I check the Corkboard?"))
    .addSubcommand((subcommand) => subcommand.setName("crafter").setDescription("How do I find a raid crafter?"))
    .addSubcommand((subcommand) =>
        subcommand.setName("housing").setDescription("How do I get a house in the guild neighborhood?"),
    )
    .addSubcommand((subcommand) => subcommand.setName("logs").setDescription("How do I check the logs?"))
    .addSubcommand((subcommand) => subcommand.setName("loot").setDescription("How do I know what to roll for loot?"))
    .addSubcommand((subcommand) => subcommand.setName("schedule").setDescription("How do I find the raid schedule?"))
    .addSubcommand((subcommand) => subcommand.setName("signup").setDescription("How do I sign up to raid?"))
    .addSubcommand((subcommand) => subcommand.setName("strats").setDescription("How do I find the raid strats?"))
    .addSubcommand((subcommand) => subcommand.setName("trb").setDescription("How do I join the TRB channel in game?"));

export async function execute(interaction) {
    switch (interaction.options.getSubcommand()) {
        case "addons":
            let addonsEmbed = new EmbedBuilder()
                .setColor(0xffffff)
            ...
            break;
        case "attendance":
            let absenceEmbed = new EmbedBuilder()
                .setColor(0xffffff)
        ...
        case "corkboard":
            ...
            break;
        case "crafter":
            let craftEmbed = new EmbedBuilder().setColor(0xffffff).setTitle("How do I find a raid crafter?").addFields({
                name: " ",
            ...
            break;
        case "housing":
            let houseEmbed = new EmbedBuilder()
                .setColor(0xffffff)
                .setTitle("How do I get a house in the guild neighborhood?")
                .addFields({
                    name: " ",
                    value: "\nAll guild members are guaranteed a house! If you are looking to pick out a plot, the first thing to do is check with Evie/Ntayu and she will help you find one that hasn’t been claimed yet!\nAfter that, you can just zone in to the Guild Neighborhood, find the one with your number, and click on the sign to purchase it! There is a tutorial in game in the own center if you need more tips on how to get started!",
                    inline: true,
                });

            interaction.reply({ embeds: [houseEmbed], flags: MessageFlags.Ephemeral });
            break;
        case "logs":
            let logsEmbed = new EmbedBuilder().setColor(0xffffff).setTitle("How do I check the logs?").addFields({
                name: " ",
            ...
            break;
        case "loot":
            let lootEmbed = new EmbedBuilder()
                .setColor(0xffffff)
            ...
            break;
        case "schedule":
            let schedEmbed = new EmbedBuilder()
                .setColor(0xffffff)
            ...
            break;
        case "signup":
            let signupEmbed = new EmbedBuilder().setColor(0xffffff).setTitle("How do I sign up to raid?").addFields({
                name: " ",
            ...
            break;
        case "strats":
            let stratsEmbed = new EmbedBuilder()
                .setColor(0xffffff)
