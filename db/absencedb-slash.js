...
class AttendanceTools {
    ...
    generateResponse(name, this_command, start, end, reason) {
        ...
        if (this_command === 'absent') {
            if (start != end) {
                client.channels.cache.get(`${process.env.attendance_channel}`).send(`${name} will be absent from ${friendlyStart} until ${friendlyEnd}. They commented: ${reason}`);
            } else {
                client.channels.cache.get(`${process.env.attendance_channel}`).send(`${name} will be absent on ${friendlyStart}. They commented: ${reason}`);
            }
        }
        if (this_command === 'late') {
            if (start != end) {
                client.channels.cache.get(`${process.env.attendance_channel}`).send(`${name} will be late to raid from ${friendlyStart} until ${friendlyEnd}. They commented: ${reason}`);
            } else {
                client.channels.cache.get(`${process.env.attendance_channel}`).send(`${name} will be late on ${friendlyStart}. They commented: ${reason}`);
            }
