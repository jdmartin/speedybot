...
class DatabaseTools {
    ...
    addAbsence(message, args) {
        ...
        if (this.processDates(message, startDate, endDate)) {
            message.author.send("Ok, I've got the date(s). If you'd like to add a comment, reply to me in the next five minutes...or, hit enter to skip.");
    ...
    tardy(message, args) {
        ...
        if (isValid(parseISO(startDate))) {
            message.author.send("Ok, I've got the date. If you'd like to add a comment, or if you want to let us know when you're coming, reply to me in the next five minutes...or, hit enter to skip.");
            message.author.createDM().then(dmchannel => {
                const collector = new Discord.MessageCollector(dmchannel, m => m.author.id === message.author.id, {
                    max: 1,
                    time: 300000
